...
(define bmap (read-bitmap "vt220.png"))
...
(define (pixel-row->int pr)
  (for/sum ([(bit idx) (in-indexed (reverse pr))]
            #:unless (zero? bit))
           (expt 2 idx)))
...
(define pixel (x:x:array #:length 4 #:type x:uint8
                         #:post-decode (λ (val) (if (zero? (fourth val)) 1 0))))
(define pixel-row-int (x:x:array #:length 8
                                 #:type pixel
                                 #:post-decode pixel-row->int))
(define pixel-cell (x:x:array #:type pixel-row-int #:length cell-height))

(define cells
  (for*/vector ([col (in-range 18)]
                [row (in-range 16)])
               (define x (+ (* col (+ cell-width 2)) x-offset))
               (define y (+ (* row (+ cell-height 2)) y-offset))
               (define bs (make-bytes (* 4 cell-width cell-height)))
               (send bmap get-argb-pixels x y cell-width cell-height bs)
               (x:decode pixel-cell bs)))
...
(define (make-svgs matrix [cell-proc default-cell-proc])
  (append
   #;(for/list ([(row y) (in-indexed matrix)])
               `(line ((x1 "0")
                       (y1 ,(format "~a" (* yunit y)))
                       (x2 ,(format "~a" (* xunit (length row))))
                       (y2 ,(format "~a" (* yunit y)))
                       (class "bg-line")
                       #;(stroke-dasharray ,(make-dasharray)))))
   (append*
    (for/list ([(row y) (in-indexed matrix)])
...
(define (string->svg #:width [width #f] . strs)
  ...
  (html->xexpr
   ◊string-append{
 <div class="svg"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="-15 -20 ◊(number->string (+ 50 (* 100 #R max-line-chars))) ◊(number->string (+ 20 (* 10 yunit line-count)))" >
